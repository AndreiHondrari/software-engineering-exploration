...
LinkedList mergeSort(LinkedList * list) {
  const unsigned int LIST_SIZE = getListSize(list);
  ...
  const unsigned int MID_INDEX = LIST_SIZE / 2;

  // split in halves
  SplitLinkedList resultSplit = splitList(&result, MID_INDEX);
  LinkedList left = resultSplit.left;
  LinkedList right = resultSplit.right;

  // merge sort recursively
  LinkedList sortedLeft = mergeSort(&left);
  LinkedList sortedRight = mergeSort(&right);

  // clear danglight left/right lists and source list
  clearListMemory(&left);
  clearListMemory(&right);
  result.head = NULL;

  // merge left and right
  result = mergeInOrder(&sortedLeft, &sortedRight);
  clearListMemory(&sortedLeft);
  clearListMemory(&sortedRight);
