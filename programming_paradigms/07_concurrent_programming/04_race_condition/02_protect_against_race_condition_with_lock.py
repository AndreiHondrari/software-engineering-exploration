...
def do_illegal_race(lock: threading.Lock) -> None:
    ...
    print(f"[{this_thread.name}] try change the global", flush=True)
    with lock:
        global_variable = 666
...
def do_something(lock: threading.Lock) -> None:
    ...
    try:
        if global_variable < 20:
            ...
            print(
                f"[{this_thread.name}] use the locked global in local",
                flush=True
            )
    ...
    print(
        f"[{this_thread.name}] our non-corrupted local: {local_variable}",
        flush=True
    )
